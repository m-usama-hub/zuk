            ...
            <div class="card-body">
                ...
                @if ($getcat ?? '')
                    @php
                        $buttontext = 'Update';
                    @endphp
                    <form action="{{ route('category.update', $getcat->id) }}?category_of=item" method="POST">
                        @method('patch')
                        @csrf
                    @else
                        @php
                            $buttontext = 'Add';
                        @endphp
                        <form action="{{ route('category.store') }}?category_of=item" method="POST">
                            @csrf
                @endif
                                ...
                                @foreach ($category as $k => $role)
                                    <tr>
                                        ...
                                        <td class="text-nowrap">
                                            ...
                                            @can('category-edit')
                                                <a href="{{ route('category.edit', $role->id) }}?category_of=item"
